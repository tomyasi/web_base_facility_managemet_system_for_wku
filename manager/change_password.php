...
if (isset($_POST['re_password'])) {
    ...
    $password_query = mysqli_query($con, "SELECT * from eaccount where emp_id='$manager_id'");
    $password_row = mysqli_fetch_array($password_query);
    $database_password = $password_row['password'];
    if ($database_password == $old_pass) {

        if ($new_pass == $re_pass) {
            $update_pwd = mysqli_query($con, "UPDATE eaccount set password='" . md5($new_pass) . "' where emp_id='$manager_id'");
